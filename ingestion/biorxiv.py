...
def fetch_range(start: date, end: date, save_dir: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if save_dir is not None:
        ensure_dir(save_dir)
        write_text(Path(save_dir) / f"biorxiv_{start.isoformat()}_{end.isoformat()}.json", resp.text)
...
if __name__ == "__main__":
    out = Path("data/raw/biorxiv")
    data = fetch_recent(save_dir=out)
    print(f"Fetched {len(data.get('collection', []))} biorxiv records (recent)")
