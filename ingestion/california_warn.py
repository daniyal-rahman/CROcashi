...
from ingestion.utils.staging_loader import StagingLoader, warn_notice_id_extractor
...
def fetch_recent_warn_notices(
    ...
    """
    Fetch recent California WARN notices.
    ...
    try:
        ...
        # Find WARN notice links and parse them
        warn_notices = []
        links_found = []
        ...
        for a in soup.find_all("a", href=True):
            ...
            href = a.get("href", "")
            if "warn" in text.lower() or "warn" in href.lower() or "notice" in text.lower():
        ...
        if save_dir is not None:
            ensure_dir(save_dir)
            write_text(Path(save_dir) / "california_warn.html", html)
            import json
            write_text(Path(save_dir) / "california_warn.json", json.dumps(result, indent=2, default=str))
        
        # Load to staging if requested
        if load_to_staging and warn_notices:
            loader = StagingLoader('california_warn')
            stats = loader.load_records(
                warn_notices,
                id_extractor=warn_notice_id_extractor,
...
def _parse_warn_notice(client: HttpClient, url: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Extract company name (usually in first line or heading)
        company_name = None
        ...
        # If not in heading, look for company name patterns in text
        if not company_name:
            # Look for patterns like "Company Name" or "Company Name, Inc."
        ...
        if not company_name:
            # If we can't extract company name, skip this notice
            return None
        ...
        if not notice_id:
            if notice_date and company_name:
                notice_id = generate_hash_id('CA-WARN', notice_date.replace('/', '-'), company_name)
            else:
                notice_id = generate_abstract_id('CA-WARN', url)
        ...
        return {
            ...
            'reason': reason,
            'notice_url': url,
            'notice_text': text[:50000] if len(text) > 50000 else text,  # Truncate if too long
        }
...
if __name__ == "__main__":
    out = Path("data/raw/california_warn")
    result = fetch_recent_warn_notices(limit=20, save_dir=out, load_to_staging=True)
    print(f"Fetched {result.get('notices_parsed', 0)} California WARN notices")
