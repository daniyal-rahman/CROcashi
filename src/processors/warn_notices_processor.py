"""
WARN Notices processor for extracting financial distress signals via layoffs.
...
class WARNNoticesProcessor(BaseProcessor):
    """
    Processor for California WARN Notices.
    
    WARN Notices provide:
    ...
    def extract_entities(self, raw_data: Dict[str, Any]) -> Dict[str, List[ExtractedEntity]]:
        ...
        try:
            ...
            logger.error(f"Error extracting WARN notice data: {e}")
    ...
    def extract_relationships(
        ...
        # This is different from other processors which create RegulatoryEvent entities
        try:
    ...
    def _extract_company(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        ...
        if not company_name:
            logger.warning("No company name found in WARN notice")
            return None
