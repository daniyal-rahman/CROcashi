"""
FDA Expanded Access processor for extracting compassionate use data.

Extracts:
- Company entities (sponsors)
- Drug entities (expanded access products)
- Regulatory events (expanded access authorizations)
"""
import logging
import re
...
class FDAExpandedAccessProcessor(BaseProcessor):
    """
    Processor for FDA Expanded Access (Compassionate Use) data.
    
    Similar to EUA but for compassionate use.
    """
    
    SOURCE_NAME = "fda_expanded_access"
    
    def get_source_identifier(self, raw_data: Dict[str, Any]) -> str:
        """Get unique identifier from FDA Expanded Access data."""
        return raw_data.get('url') or raw_data.get('eind_id') or raw_data.get('raw_text', '')[:100] or ''
    
    def extract_entities(self, raw_data: Dict[str, Any]) -> Dict[str, List[ExtractedEntity]]:
        """Extract entities from FDA Expanded Access record."""
        ...
        try:
            ...
            logger.error(f"Error extracting FDA Expanded Access data: {e}")
    ...
    def _extract_company(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        ...
        company = ExtractedEntity(
            ...
            name=company_name,
            identifiers={},
            context={'role': 'sponsor', 'access_type': 'expanded_access'},
    ...
    def _extract_drug(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        ...
        drug = ExtractedEntity(
            ...
            name=drug_name,
            identifiers={},
            context={'expanded_access': True, 'authorization_date': raw_data.get('authorization_date')},
            source_name=self.SOURCE_NAME,
            source_identifier=self.get_source_identifier(raw_data),
    ...
    def _extract_regulatory_event(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        """Extract regulatory event (expanded access authorization)."""
        ...
        event_name = f"FDA Expanded Access Authorization"
        if company_name and drug_name:
            event_name = f"FDA Expanded Access: {company_name} - {drug_name}"
        ...
        event = ExtractedEntity(
            ...
            context={
                ...
                'regulatory_body': 'FDA',
                'country': 'US',
                'authorization_type': 'expanded_access'
            },
            source_name=self.SOURCE_NAME,
            source_identifier=self.get_source_identifier(raw_data),
            raw_data=raw_data
