"""
EMA EPAR processor for extracting EU regulatory approval data.

Extracts:
- Company entities (marketing authorization holders)
- Drug entities (approved medicines)
- Regulatory events (EMA approvals)
"""
import logging
import re
...
class EMAEPARProcessor(BaseProcessor):
    """
    Processor for EMA EPAR (European Public Assessment Report) data.
    
    EMA EPAR provides:
    - Company information (marketing authorization holder)
    - Drug names (approved medicines)
    - Approval dates
    - Regulatory event (EMA approval)
    """
    
    SOURCE_NAME = "ema_epar"
    
    def get_source_identifier(self, raw_data: Dict[str, Any]) -> str:
        """Get unique identifier from EMA EPAR data."""
        return raw_data.get('epar_id') or raw_data.get('medicine_name', '')[:100] or ''
    
    def extract_entities(self, raw_data: Dict[str, Any]) -> Dict[str, List[ExtractedEntity]]:
        """Extract entities from EMA EPAR record."""
        ...
        try:
            ...
            logger.error(f"Error extracting EMA EPAR data: {e}")
    ...
    def _extract_company(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        """Extract company (marketing authorization holder) from EPAR data."""
        company_name = raw_data.get('marketing_authorization_holder') or raw_data.get('company_name', '')
        
        if not company_name:
            logger.warning("No company name found in EMA EPAR record")
            return None
        ...
        company = ExtractedEntity(
            ...
            identifiers={},
            context={
                'role': 'marketing_authorization_holder',
                'regulatory_body': 'EMA'
            },
    ...
    def _extract_drug(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        """Extract drug from EPAR data."""
        drug_name = raw_data.get('medicine_name') or raw_data.get('drug_name', '')
        
        if not drug_name:
            logger.warning("No drug name found in EMA EPAR record")
            return None
    ...
    def _extract_regulatory_event(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        """Extract regulatory event (EMA approval) from data."""
        ...
        drug_name = raw_data.get('medicine_name', '')
        event_name = f"EMA Approval: {drug_name}" if drug_name else "EMA Approval"
    ...
    def validate_extraction(self, entities: Dict[str, List[ExtractedEntity]]) -> bool:
        ...
        return bool(entities.get('regulatory_events') or entities.get('drugs'))
