"""
TGA Australia processor for extracting Australian regulatory approval data.

Extracts:
- Company entities (marketing authorization holders)
- Drug entities (approved medicines)
- Regulatory events (TGA approvals)
"""
import logging
...
class TGAAustraliaProcessor(BaseProcessor):
    """
    Processor for TGA (Therapeutic Goods Administration, Australia) data.
    
    Similar to FDA Drugs@FDA but for Australian approvals.
    """
    
    SOURCE_NAME = "tga_australia"
    
    def get_source_identifier(self, raw_data: Dict[str, Any]) -> str:
        """Get unique identifier from TGA data."""
        return raw_data.get('artg_id') or raw_data.get('product_name', '')[:100] or ''
    
    def extract_entities(self, raw_data: Dict[str, Any]) -> Dict[str, List[ExtractedEntity]]:
        """Extract entities from TGA record."""
        ...
        try:
            ...
            logger.error(f"Error extracting TGA data: {e}")
    ...
    def _extract_company(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        ...
        company_name = raw_data.get('company_name') or raw_data.get('sponsor', '')
        ...
        company = ExtractedEntity(
            ...
            name=company_name,
            identifiers={},
            context={'role': 'marketing_authorization_holder', 'regulatory_body': 'TGA'},
    ...
    def _extract_drug(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        ...
        drug = ExtractedEntity(
            ...
            name=drug_name,
            identifiers={'artg_id': raw_data.get('artg_id')},
            context={'regulatory_body': 'TGA', 'approval_date': raw_data.get('approval_date')},
            source_name=self.SOURCE_NAME,
            source_identifier=self.get_source_identifier(raw_data),
    ...
    def _extract_regulatory_event(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        ...
        event_name = f"TGA Approval: {drug_name}" if drug_name else "TGA Approval"
