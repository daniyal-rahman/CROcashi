"""
EMA Clinical Trials processor for extracting EU clinical trial data.

Extracts:
- Trial entity (EudraCT number, phase, status, dates)
...
class EMATrialsProcessor(BaseProcessor):
    """
    Processor for EMA Clinical Trials Register data.
    
    Similar to ClinicalTrials.gov but for EU trials.
    """
    
    SOURCE_NAME = "ema_trials"
    
    def get_source_identifier(self, raw_data: Dict[str, Any]) -> str:
        """Get unique identifier from EMA trials data."""
        return raw_data.get('eudract_number') or raw_data.get('trial_id') or raw_data.get('id', '')
    
    def extract_entities(self, raw_data: Dict[str, Any]) -> Dict[str, List[ExtractedEntity]]:
        """Extract entities from EMA trials record."""
        ...
        try:
            ...
            logger.error(f"Error extracting EMA trials data: {e}")
    ...
    def extract_relationships(
        ...
        # Trial-drug and trial-disease relationships
        for drug_id in resolved_entities.get('drugs', []):
            if isinstance(drug_id, UUID):
    ...
    def _extract_sponsors(self, raw_data: Dict[str, Any]) -> List[ExtractedEntity]:
        """Extract sponsor entities."""
        sponsors = []
        
        sponsor_name = raw_data.get('sponsor') or raw_data.get('sponsor_name', '')
        if sponsor_name:
            entity_type = EntityType.COMPANY
            if any(keyword in sponsor_name.lower() for keyword in ['university', 'hospital', 'institute']):
            ...
            sponsor = ExtractedEntity(
                ...
                identifiers={},
                context={'role': 'sponsor', 'registry': 'EMA'},
                source_name=self.SOURCE_NAME,
