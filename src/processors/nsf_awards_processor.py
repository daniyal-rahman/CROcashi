"""
NSF Awards processor for extracting research grant data.
...
class NsfAwardsProcessor(BaseProcessor):
    """
    Processor for NSF Awards (research grants) data.
    ...
    def get_source_identifier(self, raw_data: Dict[str, Any]) -> str:
        """Get unique identifier from NSF Awards data."""
        return raw_data.get('award_id') or raw_data.get('award_number') or raw_data.get('id', '')
    
    def extract_entities(self, raw_data: Dict[str, Any]) -> Dict[str, List[ExtractedEntity]]:
        """Extract entities from NSF Awards record."""
        ...
        try:
            ...
            org_name = raw_data.get('organization_name') or raw_data.get('org_name') or raw_data.get('institution', '')
            if org_name:
            ...
            logger.error(f"Error extracting NSF Awards data: {e}")
    ...
    def extract_relationships(
        ...
        # NSF grants don't typically create entity relationships
        return []
    ...
    def _extract_company(self, raw_data: Dict[str, Any]) -> Optional[ExtractedEntity]:
        ...
        company = ExtractedEntity(
            ...
            identifiers={},
            context={'role': 'grant_recipient', 'source': 'nsf_awards'},
