...
Entry point for running the OA worker that processes PUBMED_OA tasks.
Handles open access full-text document retrieval.
"""
...
def setup_logging(env: str):
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f'logs/oa_worker_{env}.log')
        ]
    )
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='PubMed OA Worker')
    ...
    logger.info(f"Starting PubMed OA Worker (env: {args.env})")
    ...
    try:
        ...
        # Task queue service
        worker_id = args.worker_id or f"oa_worker_{args.env}_{os.getpid()}"
        queue_service = TaskQueueService(worker_id=worker_id)
        
        # Initialize client and worker
        async with PubMedClient(client_config) as client:
            # Create worker
            worker = OAWorker(
                client=client,
            ...
            logger.info(f"Initialized OA worker with ID: {worker_id}")
    ...
    logger.info("OA worker shutdown complete")
